...
//    if($json->success == "true"){
      ...
      if($mail["mail"] != $_POST["mail"]){
        $idsearch = mymysql("vc","vchello")->query("SELECT * FROM vc.account WHERE id = 0");
        $id = $idsearch->fetch();
        $nowid = intval($id["time"]) + 1;
        $_POST["pass"] = passcrypt($_POST["pass"]);
        $already = mymysql("vc","vchello")->prepare("INSERT INTO vc.account (mail,pass,id,time) VALUES (:mail,:pass,:id,:time)");
        $already->execute(array(":mail"=>$_POST["mail"],":pass"=>$_POST["pass"],":id"=>$nowid,":time"=>time()));
        $idupdate = mymysql("vc","vchello")->prepare("UPDATE vc.account SET time = :nowid WHERE id = 0");
        $idupdate->execute(array(":nowid"=>$nowid));
        alert("info","完了しました。ログインしてください。");
      }else{
        alert_wide("danger","既にそのメールアドレスで登録されています。");
      }
//    }
//  }else{
...
?>
<div class="panel panel-success" style="margin: 30px 10px;">
